...
BlockAssembler::BlockAssembler(const CChainParams& params, const Options& options) : chainparams(params)
{
    ...
    nBlockMaxSize = std::max<size_t>(1000, std::min<size_t>(dgpMaxBlockSerSize - 1000, options.nBlockMaxSize));
...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx, bool fProofOfStake, int64_t* pTotalFees, int32_t txProofTime, int32_t nTimeLimit)
{
    ...
    if(gArgs.GetArg("-staker-min-tx-gas-price", false)) {
    ...
    LogPrint(BCLog::BENCH, "miner", "CreateNewBlock(): total size: %u block weight: %u txs: %u fees: %ld sigops %d\n", nSerializeSize, GetBlockWeight(*pblock), nBlockTx, nFees, nBlockSigOpsCost);
...
void BlockAssembler::addPackageTxs(int &nPackagesSelected, int &nDescendantsUpdated, uint64_t minGasPrice = 0)
{
...
bool CheckStake(const std::shared_ptr<const CBlock> pblock, CWallet& wallet)
{
    ...
    //// debug print
    LogPrint(BCLog::BENCH, "miner", "CheckStake() : new proof-of-stake block found  \n  hash: %s \nproofhash: %s  \ntarget: %s\n", hashBlock.GetHex(), proofHash.GetHex(), hashTarget.GetHex());
    LogPrint(BCLog::BENCH, "miner", "%s\n", pblock->ToString());
    LogPrint(BCLog::BENCH, "miner", "out %s\n", FormatMoney(pblock->vtx[1]->GetValueOut()));
            ...
            for(uint32_t i=beginningTime;i<beginningTime + MAX_STAKE_LOOKAHEAD;i+=STAKE_TIMESTAMP_MASK+1) {
                ...
                if (SignBlock(pblock, *pwallet, nTotalFees, i)) {
                    ...
                    if(pindexPrev->GetBlockHash() != beginningHash){
                        ...
                        LogPrint(BCLog::BENCH, "staker", "ThreadStakeMiner(): Valid future PoS block was orphaned before becoming valid");
                        break;
                    }
                    ...
                    if (SignBlock(pblockfilled, *pwallet, nTotalFees, i)) {
                        ...
                        while(!validBlock) {
                            ...
                            if (pblockfilled->GetBlockTime() <= pindexPrev->GetBlockTime() ||
                                ...
                                LogPrint(BCLog::BENCH, "staker", "ThreadStakeMiner(): Valid PoS block took too long to create and has expired");
