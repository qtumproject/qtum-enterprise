...
bool AddMPoSScript(std::vector<CScript> &mposScriptList, int nHeight, const Consensus::Params& consensusParams)
{
    ...
    if(!pblockindex)
    {
        LogPrint(BCLog::ALL, "Block index not found\n");
        return false;
    }
    ...
    if(pblockindex->IsProofOfStake())
    {
        ...
        {
            LogPrint(BCLog::ALL, "Fail to solve script for mpos reward recipient\n");
    ...
    else
    {
        ...
        LogPrint(BCLog::ALL, "The block is not proof-of-stake\n");
        return false;
    }
...
bool CreateMPoSOutputs(CMutableTransaction& txNew, int64_t nRewardPiece, int nHeight, const Consensus::Params& consensusParams)
{
    ...
    {
        LogPrint(BCLog::ALL, "Fail to get the list of recipients\n");
        return false;
    }
                                ...
                                int& nChangePosInOut, std::string& strFailReason, const CCoinControl& coin_control, bool sign, CAmount nGasFee, bool hasSender)
{
    ...
    if(hasSender && coin_control != CCoinControl() && coin_control.HasSelected()){
    ...
    unsigned int nBytes;
    {
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                if (hasSender && coin_control != CCoinControl() && coin_control.HasSelected())
                {
                    for (std::vector<std::pair<const CWalletTx*,unsigned int>>::size_type i = 0 ; i != vCoins.size(); i++){
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, const CAmount& nTotalFees, uint32_t nTimeBlock, CMutableTransaction& tx, CKey& key)
{
    ...
    for(const std::pair<const CWalletTx*, unsigned int>& pcoin : setCoins)
    {
        ...
        {
            // Found a kernel
            LogPrint(BCLog::ALL, "CreateCoinStake : kernel found\n");
            ...
            {
                LogPrint(BCLog::ALL, "CreateCoinStake : failed to parse kernel\n");
                break;
            }
            LogPrint(BCLog::ALL, "CreateCoinStake : parsed kernel type=%d\n", whichType);
            if (whichType != TX_PUBKEY && whichType != TX_PUBKEYHASH)
            {
                LogPrint(BCLog::ALL, "CreateCoinStake : no support for kernel type=%d\n", whichType);
            ...
            if (whichType == TX_PUBKEYHASH) // pay to address type
            {
                ...
                {
                    LogPrint(BCLog::ALL, "CreateCoinStake : failed to get key for kernel type=%d\n", whichType);
            ...
            if (whichType == TX_PUBKEY)
            {
                ...
                {
                    LogPrint(BCLog::ALL, "CreateCoinStake : invalid key for kernel type=%d\n", whichType);
                    break; // keys mismatch
                }
            ...
            LogPrint(BCLog::ALL, "CreateCoinStake : added kernel type=%d\n", whichType);
            fKernelFound = true;
            break;
        }
